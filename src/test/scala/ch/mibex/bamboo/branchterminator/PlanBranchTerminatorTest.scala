...
class PlanBranchTerminatorTest extends Specification with Mockito {
  ...
  class PlanBranchContext extends Scope {
  ...
  "plan branch terminator" should {
    ...
    "not delete any plan branches when no matching ones were found" in new PlanBranchContext {
      (cachedPlanManager.getPlansWithRepository(any[PlanRepositoryIndex.Query])
        returns List[ImmutableChain]().asJava)
    ...
    "not delete found plan branches not ending with a number in their key" in new PlanBranchContext {
      val planBranch = mock[ImmutableChain]
    ...
    "delete all plan branches referencing deleted branch" in new PlanBranchContext {
      ...
      (cachedPlanManager.getPlansWithRepository(any[PlanRepositoryIndex.Query])
        returns List(planBranch1, planBranch2).asJava)
